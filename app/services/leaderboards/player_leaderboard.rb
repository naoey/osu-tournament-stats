module PlayerLeaderboard
  ...
  def get_leaderboard(limit = nil, offset = 0, order: 'score', ascending: true)
    ...
    q = MatchScore
      .joins(:player)
      .select('players.name AS player_name, AVG(match_scores.score) AS score, ROUND(AVG(match_scores.accuracy) * 100, 2) AS average_acc, COUNT(*) AS play_count')
    ...
    q
      .all
      .map { |s| { player_name: s.player_name, score: s.score, average_acc: s.average_acc, play_count: s.play_count } }
  end
end
