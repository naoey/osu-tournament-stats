...
class Ban < CommandBase
  ...
  def make_response
    ...
    unless target_db_player.ban_status == Player.ban_status[:none]
      return @event.respond("#{mentioned_member.name} is already banned")
    end

    ban_type = Player.ban_status[@options[:type].to_sym] || Player.ban_status[:hard]
  ...
  def execute_discord_action(member, ban_type, reason)
    ...
    if ban_type == Player.ban_status[:soft] && verified_role_id && member.role?(verified_role_id)
      member.remove_role(verified_role_id)
    elsif ban_type == Player.ban_status[:hard]
