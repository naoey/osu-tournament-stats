...
module RegistrationCommands
  ...
  application_command(:set_register_channel, aliases: [], description: 'Set channel for registration') do |event, *args|
    SetChannel.new(event, *args).response
  end

  application_command(:set_verification_log_channel, aliases: [],
  ...
  application_command(:set_verified_role, aliases: [], description: 'Set the role to be applied to verified users') do |event, *args|
    SetVerifiedRole.new(event, *args).response
  end

  application_command(
    :register,
    description: 'Link your osu! ID to your Discord ID through KelaBot to gain access to the server.',
  ) do |event, *args|
    Register.new(event, *args).response
  end

  application_command(:unregister, aliases: [], description: 'Unlinks Discord user from osu! user. Admin only.') do |event, *args|
    Unregister.new(event, *args).response
  end

  application_command(:whois, aliases: [:who], description: 'Display linked osu! profile') do |event, *args|
    Whois.new(event, *args).response
  end

  application_command(:ban, aliases: [], description: 'Ban a user') do |event, *args|
    Ban.new(event, *args).response
  end

  application_command(:unban, aliases: [], description: 'Unban a user') do |event, *args|
